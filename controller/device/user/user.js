...
})
{
  ...
  const findAllUser = async ({
    ...
    try {
      let query = {};
      let options = {};
      makeUser(data, 'findFilterKeys',true);
      ...
      let result = await userService.findMany(query, options);
      if (result){
  ...
  const getUserCount = async (data) => {
    try {
      if (data && data.where){
        makeUser(data, 'findFilterKeys');
        where = data.where;
      }

      let result = await userService.count(where);
      if (result){
  ...
  const softDeleteManyUser = async (ids,loggedInUser) => {
    try {
      if (ids){
        ...
        let data = await deleteDependentService.softDeleteUser(query, updateBody);
  ...
  const softDeleteUser = async ({
    ...
    try {
      if (pk){
        ...
        updatedUser = await deleteDependentService.softDeleteUser(query, updateBody);            
  ...
  const findUserByPk = async (pk,body = {}) => {
    try {
      ...
      if (body && body.select && body.select.length) {
        makeUser(body, 'findFilterKeys',true);
