...
exports.schemaKeys = joi.object({
  ...
  journal_id: joi.number().integer().allow(0),
  amount: joi.number().allow(0),
  reff_model: joi.string().allow(null).allow(''),
  reff_model_id: joi.number().integer().allow(0),
...
exports.findFilterKeys = joi.object({
  ...
  ...Object.fromEntries(
    keys.map(key => [key, joi.object({
      ...
      journal_id: joi.alternatives().try(joi.array().items(),joi.number().integer(),joi.object()),
      amount: joi.alternatives().try(joi.array().items(),joi.number(),joi.object()),
      reff_model: joi.alternatives().try(joi.array().items(),joi.string(),joi.object()),
      reff_model_id: joi.alternatives().try(joi.array().items(),joi.number().integer(),joi.object()),
